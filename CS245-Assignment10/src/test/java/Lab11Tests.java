...
class Lab11Tests {
	...
	public void testClear() {
		try {
			numElements.set(bst, 1);
			root.set(bst, nodeConstructor.newInstance(37, null, null, null));
			bst.clear();
	...
	public void testIterator() {
		BinarySearchTree.Iterator itr = bst.iterator();
		Set<Integer> values = new TreeSet<Integer>();
	...
	private void grumpy(BinarySearchTree<Integer> b, Integer v) {
        try {
			root.set(b, sleepy(root.get(b), v, null, b));
		} catch (IllegalArgumentException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IllegalAccessException e) {
    ...
    private Object sleepy(Object n, Integer v, Object pa, Object b) {
    		try {
	        if (n == null) {
	            n = nodeConstructor.newInstance(v, null, null, null);
	            parent.set(n, pa);
	            numElements.set(b, (Integer)(numElements.get(b))+1);
	        } else if (((Integer) data.get(n)).compareTo(v) > 0) {
	            left.set(n, sleepy(left.get(n), v, n, b));
	        } else if (((Integer) data.get(n)).compareTo(v) < 0) {
	            right.set(n, sleepy(right.get(n), v, n, b));
	        }
    		} catch (IllegalArgumentException e) {
    			// TODO Auto-generated catch block
    			e.printStackTrace();
    ...
    private void happy(BinarySearchTree<Integer> b, Integer v) {
		try {
			root.set(b, dopey(root.get(b), v, null, b));
		} catch (IllegalArgumentException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IllegalAccessException e) {
    ...
    private Object dopey(Object n, Integer v, Object pa, Object b) {
		try {
        if (n == null) {
            return null;
        } else if (((Integer)data.get(n)).compareTo(v) > 0) {
            left.set(n, dopey(left.get(n), v, n, b));
        } else if (((Integer)data.get(n)).compareTo(v) < 0) {
            right.set(n, dopey(right.get(n), v, n, b));
        } else {  
            if (right.get(n) == null) {
            		if(left.get(n) != null) {
            			parent.set(left.get(n), pa);
            		}
            		numElements.set(b, (Integer)(numElements.get(b))-1);
                return left.get(n);    
            } else if (left.get(n) == null) {
	        		if(right.get(n) != null) {
	        			parent.set(right.get(n), pa);
	        		}
	        		numElements.set(b, (Integer)(numElements.get(b))-1);
                return right.get(n);   
            } else {
                data.set(n, bashful(left.get(n)));
                left.set(n, dopey(left.get(n), (Integer)data.get(n), n, b));
            }
        }
		} catch (IllegalArgumentException e) {
			// TODO Auto-generated catch block
    ...
    private Integer bashful(Object n) {
    		try {
	        if (right.get(n) == null) {
	            return (Integer)data.get(n);
	        } else {
	            return bashful(right.get(n));
	        }
    		} catch (IllegalArgumentException e) {
    			// TODO Auto-generated catch block
    			e.printStackTrace();
